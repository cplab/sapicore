...
from tree_config.yaml import register_torch_yaml_support, register_numpy_yaml_support

from sapicore.model import SapicoreModel
from sapicore.engine.neuron import AnalogNeuron
from sapicore.engine.synapse import SapicoreSynapse
from sapicore.pipeline import PipelineBase
from sapicore.learning import SapicoreLearning
from sapicore.utils.logging import (
...
class SimpleLearning(SapicoreLearning):
    ...
    def apply_learning(self, pre_neuron: SimpleNeuron, synapse: SimpleSynapse, post_neuron: SimpleNeuron, **kwargs):
        """Called to apply learning to the given neurons."""
        synapse.weight *= torch.abs(pre_neuron.activation) * torch.abs(post_neuron.activation) * self.attenuation
...
class MyModel(SapicoreModel):
    ...
    _config_children_ = {"neuron 1": "neuron_1", "synapse": "synapse", "neuron 2": "neuron_2", "learning": "learning"}
...
class SimplePipeline(PipelineBase):
    ...
    def init_model(self):
        ...
        debug_logging = load_save_get_loggable_properties(model, debug_logging_filename, default_value=True)
        ...
        self.property_arrays = writer.get_property_arrays("simple_example", debug_logging)
        ...
        self.tensor_loggable_properties = get_loggable_properties(model, log_opts)
        self.tensorboard_writer = SummaryWriter(log_dir=join(self.root_path, "tensorboardx"))
    ...
    def run_iteration(self, i):
        ...
        with torch.no_grad():
            ...
            log_tensor_board(
                self.tensorboard_writer, self.tensor_loggable_properties, global_step=i, prefix="simple_example"
            )
...
if __name__ == "__main__":
    ...
    with NixLogReader(join(pipeline.root_path, "debug_logging.h5")) as reader:
        ...
        print("Logged properties: ", reader.get_experiment_property_paths("simple_example"))

        print(
            "Logged neuron 1 intensity: ",
            reader.get_experiment_property_data("simple_example", ("neuron_1", "intensity")),
        )
        print(
            "Logged neuron 1 activation: ",
            reader.get_experiment_property_data("simple_example", ("neuron_1", "activation")),
        )
        print(
            "Logged model activation_sum: ", reader.get_experiment_property_data("simple_example", ("activation_sum",))
        )
