...
class UCSDGasDrift(data.Dataset):
    ...
    def __init__(self, list_ids: list, labels: list, root_dir="data/drift/", batch_idx=1):
        """UCSD gas sensor data.
        ...
        self.scaled_data = pd.read_csv(root_dir + os.sep + "uscaled_array.csv", header=None).values.flatten()
        self.data = pd.read_csv(root_dir + os.sep + "batch" + str(batch_idx) + os.sep + "data.csv", header=None)
...
class Datasplitter:
    ...
    def __call__(self) -> tuple:
        ...
        for grp_idx in range(1, self.num_groups + 1):
            ...
            train_ids = np.random.choice(np.where(self.y_labels == grp_idx)[0], self.num_shots, replace=False).tolist()
            partition[grp_idx]["train"] = train_ids

            allidsforgroup = np.setdiff1d(allidsforgroup, train_ids)
            if self.isvalidationset:
                val_ids = np.random.choice(allidsforgroup, int(0.1 * len(allidsforgroup)), replace=False)
                if grp_idx > 1:
                    partition[grp_idx]["validation"] = partition[grp_idx - 1]["validation"] + val_ids.tolist()
                else:
                    partition[grp_idx]["validation"] = val_ids.tolist()
                allidsforgroup = np.setdiff1d(allidsforgroup, val_ids)
            test_ids = allidsforgroup
            if grp_idx > 1:
                partition[grp_idx]["test"] = partition[grp_idx - 1]["test"] + test_ids.tolist()
            else:
